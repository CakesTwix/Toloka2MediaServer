    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.api_client = Client(
                ...
                protocol=config.app_config[config.application_config.client][
                    "protocol"
                ],
            )
            ...
            config.logger.critical(
                f"{config.application_config.client} wrong connection details"
            )
            raise

    def add_torrent(self, torrents, category, tags, is_paused, download_dir):
        labels = [category, tags]
        return self.api_client.add_torrent(
            torrent=torrents, paused=is_paused, labels=labels, download_dir=download_dir
        ).id
