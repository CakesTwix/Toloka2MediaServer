...
def add_release_by_url(config):
    ...
    match = re.search(r"t(\d+)", proposed_url)
...
def add_release_by_name(config):
    ...
    suggested_name, suggested_codename = extract_torrent_details(torrent.name)
    title.code_name = (
        input(f"Default:{suggested_codename}. Enter the codename: ")
        or suggested_codename
    )
    # Collect additional data
    ...
    title.torrent_name = (
        input(
            f"Default: {suggested_name}. Enter the directory name for the downloaded files: "
        )
        or suggested_name
    )
    title.release_group = (
    ...
    default_meta = config.application_config.default_meta
    title.meta = (
        input(f"Default: {default_meta}. Enter additional metadata tags: ")
        or default_meta
    )
    config.operation_result = add(config, title, torrent)
...
def add_torrent(config):
    try:
        if not config.args:
            ...
            return config.operation_result
        torrent = config.toloka.download_torrent(
            f"{config.toloka.toloka_url}/{config.args.url}"
        )
